 ...
 * @category Subquery
 */
export function exceptBy<TNode, T extends TNode, K>(left: HierarchyIterable<TNode, T>, right: Queryable<T>, keySelector: (element: T) => K): HierarchyIterable<TNode, T>;
/**
 ...
 * @category Subquery
 */
export function exceptBy<T, K>(left: Queryable<T>, right: Queryable<T>, keySelector: (element: T) => K): Iterable<T>;
export function exceptBy<T, K>(left: Queryable<T>, right: Queryable<T>, keySelector: (element: T) => K): Iterable<T> {
    assert.mustBeQueryable(left, "left");
    assert.mustBeQueryable(right, "right");
    assert.mustBeFunction(keySelector, "keySelector");
    return FlowHierarchy(new ExceptByIterable(ToIterable(left), ToIterable(right), keySelector), left);
}
