...
export function takeUntil<T>(source: Queryable<T>, predicate: (element: T) => boolean): Iterable<T> {
    ...
    return FlowHierarchy(new TakeUntilIterable(ToIterable(source), predicate), source);
}

@ToStringTag("TakeUntilIterable")
class TakeUntilIterable<T> implements Iterable<T> {
...
Registry.Query.registerSubquery("takeUntil", takeUntil);